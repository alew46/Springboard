...
SELECT f.name as facility_name,m.firstname, m.surname,
	CASE WHEN b.memid != 0 THEN slots * f.membercost
	ELSE slots * f.guestcost END
	AS booking_cost
FROM Bookings AS b
...
ON b.memid = m.memid
WHERE starttime LIKE '2012-09-04%'
	AND 
		CASE WHEN b.memid != 0 THEN slots * f.membercost
		ELSE slots * f.guestcost END
	 > 30
...
SELECT b.starttime, f.name as facility_name,m.firstname, m.surname,
	CASE WHEN b.memid != 0 THEN slots * f.membercost
	ELSE slots * f.guestcost END
	AS booking_cost
FROM Bookings AS b
...
SELECT m1.firstname AS member_first, m1.surname AS member_last, m2.firstname AS recomender_first, m2.surname AS recommender_last
...
SELECT name, firstname, surname, COUNT(surname) AS member_use 
FROM Bookings 
...
USING(memid) 
WHERE memid != 0 
GROUP BY name, surname, firstname


/* Q13: Find the facilities usage by month, but not guests */

SELECT COUNT(starttime) AS monthly_use
FROM BOOKINGS